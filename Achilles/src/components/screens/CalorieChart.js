...
class CalorieChart extends Component {
                        ...
                        <Grid >
                            <Row size={0.55} >
                                <VictoryChart width={375} height={300} theme={VictoryTheme.material} minDomain={{ x: 1 }} >
                                    ...
                                    <VictoryBar style={{ data: { fill: "#269DCB" } }} data={this.state.calories} x="date" y="calories" labels={(datum) => datum.calories}
    ...
    componentWillMount() {
        ...
        fetch(`http://ec2-54-201-123-10.us-west-2.compute.amazonaws.com:5000/get_daily_health_data?userid=2134225533&start_date=12-10-2015&end_date=12-17-2015`)
            .then((result) => result.json())
            .then((result) => {
                let calories = [];
                _.map(result, (value, key) => {
                    calories.push({ 'date': moment(key).add(3, 'year').format('YYYY-MM-DD'), 'calories': value.calories })
                })
                calories.sort((a, b) => { if (a.date > b.date) { return 1; } else return -1 })
                this.setState({ calories: calories, loading: false })
            })
